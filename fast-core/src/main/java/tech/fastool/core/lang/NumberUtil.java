...
public final class NumberUtil {
    ...
    public static boolean isNumeric(CharSequence str) {
        if (StringUtil.hasText(str)) {
            if (str.length() == 1 && ((str.charAt(0) == CharUtil.PLUS || str.charAt(0) == CharUtil.MINUS))) {
                return false;
            }
            String regex = "^[-+]?(\\d+)?(\\.\\d+)?$";
            return RegexUtil.isMatch(regex, str);
        }
        return false;
    }
    ...
    public static Number createNumber(String str) throws NumberFormatException {
        ...
        if (!Character.isDigit(lastChar) && lastChar != CharUtil.DOT) {
