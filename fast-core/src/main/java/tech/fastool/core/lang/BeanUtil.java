...
public class BeanUtil {
    ...
    private static void beanToMap(Object bean, Map<String, Object> targetMap, CopyOption copyOption) {
        ...
        propertyDescriptorMap.forEach((key, pd) -> {
            Object value = ReflectUtil.invoke(bean, pd.getReadMethod());
        ...
        source.forEach((key, value) -> {
            ...
            if (value == null) {
                if (!finalCo.isIgnoreNullValue()) {
                    ReflectUtil.invoke(target, writeMethod, new Object[]{null});
                }
            } else {
        ...
        if (source instanceof Map) {
            ...
            if (target instanceof Map) {
                ...
                getPropertyDescriptorMap(source.getClass()).forEach((key, sourcePd) -> {
                    ...
                    Object value = ReflectUtil.invoke(source, sourcePd.getReadMethod());
                    if (value == null) {
                        if (!finalCo.isIgnoreNullValue()) {
                            ReflectUtil.invoke(target, targetPd.getWriteMethod(), new Object[]{null});
                        }
                    } else {
                        if (value instanceof String) {
                            String str = (String) value;
                            if (StringUtil.hasLength(str) || !finalCo.isIgnoreEmptyString()) {
                                ReflectUtil.invoke(target, targetPd.getWriteMethod(), value);
                            }
                        } else {
                            ReflectUtil.invoke(target, targetPd.getWriteMethod(), value);
                        }
