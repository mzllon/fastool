...
import tech.fastool.core.exceptions.ZipRuntimeException;
...
public final class Zips {
    ...
    public static File zip(String srcPath) {
        return zip(srcPath, Charsets.defaultCharset());
    }
    ...
    public static File zip(File srcFile) {
        return zip(srcFile, Charsets.defaultCharset());
    }
    ...
    public static File zip(File srcFile, Charset charset) {
        File zipFile = new File(srcFile.getParentFile(), Files.mainName(srcFile) + ".zip");
    ...
    public static void zip(File zipFile, File... srcFiles) {
        zip(zipFile, Charsets.defaultCharset(), srcFiles);
    }
    ...
    private static void zip(File file, String srcRootDir, ZipOutputStream out, FileFilter filter) {
        ...
        // 获取文件相对于压缩文件夹根目录的子路径
        final String subPath = Files.getRelativePath(file, srcRootDir);
        if (file.isDirectory()) {// 如果是目录，则压缩压缩目录中的文件或子目录
            final File[] files = file.listFiles();
            if (Arrays.isEmpty(files) && Strings.hasLength(subPath)) {
    ...
    private static void addDir(String path, ZipOutputStream out) {
        path = Strings.addSuffixIfNot(path, Strings.SLASH);
        try {
    ...
    private static void addFile(File file, String path, ZipOutputStream out) {
        addFile(Files.openFileInputStream(file), path, out);
    }
    ...
    private static ZipOutputStream getZipOutputStream(File zipFile, Charset charset) {
        return getZipOutputStream(Files.openFileOutputStream(zipFile), charset);
    }
    ...
    private static ZipOutputStream getZipOutputStream(OutputStream out, Charset charset) {
        ...
        return new ZipOutputStream(out, charset == null ? Charsets.defaultCharset() : charset);
    }
    ...
    private static void validateFiles(File zipFile, File... srcFiles) {
        if (zipFile.isDirectory()) {
            throw new ZipRuntimeException("Zip file '" + zipFile.getAbsoluteFile() + "' must not be a directory !");
        }
        ...
        for (File srcFile : srcFiles) {
            ...
            if (!srcFile.exists()) {
                throw new ZipRuntimeException("File '" + srcFile.getAbsolutePath() + "' not exist!");
            }
    ...
    public static File unzip(String zipFilePath) {
        return unzip(zipFilePath, Charsets.UTF_8);
    }
    ...
    public static File unzip(File zipFile, Charset charset) {
        return unzip(zipFile, new File(zipFile.getParentFile(), Files.mainName(zipFile)), charset);
    }
    ...
    public static File unzip(File zipFile, File targetDir) {
        return unzip(zipFile, targetDir, Charsets.UTF_8);
    }
    ...
    public static File unzip(ZipFile zipFile, File outFile) throws ZipRuntimeException {
        try {
    ...
    private static File buildFile(File outFile, String fileName) {
        if (!Files.isWindows()
                ...
                && fileName.lastIndexOf(Chars.SLASH, fileName.length() - 2) > 0) {
            // 在Linux下多层目录创建存在问题，/会被当成文件名的一部分，此处做处理
            // 使用/拆分路径（zip中无\），级联创建父目录
            final String[] pathParts = Strings.split(fileName, '/', true, false);
        ...
        Files.checkSlip(outFile, targetFile);
        return targetFile;
    }
    ...
    private static void write(ZipFile zipFile, ZipEntry zipEntry, File outItemFile) throws IOException {
        ...
        try {
            ...
            Files.copyStream(in, outItemFile);
        } finally {
            IOes.closeQuietly(in);
        }
    }
